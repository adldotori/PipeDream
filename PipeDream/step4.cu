...
private:
    ...
    struct aiocb * recvBefore(int batch)
    {
        if (before_socket == -1)
            return NULL;
        int cnt = 0, max_cnt = (batch_size * in * 8 - 1) / BUFSIZE + 1;
    ...
    struct aiocb * recvAfter(int batch)
    {
        if (after_socket == -1)
            return NULL;
        int cnt = 0, max_cnt = (batch_size * out * 8 - 1) / BUFSIZE + 1;
        struct aiocb *final_aiocb = (struct aiocb *)malloc(sizeof(struct aiocb));
        while (cnt < max_cnt)
        {
    ...
    void connNext(void)
    {
        ...
        while (1)
        {
            ...
            struct sockaddr_in *server = new_server(next_ip, next_port);
